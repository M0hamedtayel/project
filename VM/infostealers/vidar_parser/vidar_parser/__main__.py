"""CLI entry point for the Vidar parser."""
...
from vidar_parser.core.engine import ParserEngine


def main() -> None:
    """Main CLI entry point."""
    parser = argparse.ArgumentParser(
        description="Vidar info stealer log parser",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python -m vidar_parser --input-dir ./logs\n"
            "  python -m vidar_parser --input-dir ./logs --output-dir ./output --workers 16\n"
        ),
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        required=True,
        help="Directory containing Vidar logs (extracted dirs or zip files)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
